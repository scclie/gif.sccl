export async function onRequestPost(context) {
  ...
  try {
    ...
    await env.GIF_DB.prepare(
      "ALTER TABLE gifs ADD COLUMN tags TEXT DEFAULT '[]'",
    )
      .run()
      .catch(() => {});
    ...
    const body = new Uint8Array(
      headBuf.length + fileBuf.byteLength + footBuf.length,
    );
    body.set(headBuf, 0);
    ...
    const msg =
      err.name === "AbortError" ? "upload to storage timed out" : err.message;
